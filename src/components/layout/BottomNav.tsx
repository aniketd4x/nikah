...
export const BottomNav: React.FC = () => {
  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-2xl border-t border-cream-300 md:hidden shadow-app-sheet pb-safe select-none transition-transform duration-300 ease-in-out ${
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className="relative flex flex-col items-center justify-center flex-1 py-1 transition-all duration-200 active:scale-90 group focus:outline-none cursor-pointer"
              aria-label={item.label}
            >
              {/* Active Pill Indicator */}
              <div className={`relative px-4 py-1 rounded-full transition-all duration-200 flex items-center justify-center ${
                isActive 
                  ? 'bg-gradient-to-r from-emerald-950 via-emerald-900 to-emerald-800 text-gold-300 shadow-sm' 
                ...
                {item.label === 'Profile' ? (
                  <div className={`w-5 h-5 rounded-full overflow-hidden border transition-all ${
                    isActive ? 'border-gold-400 ring-2 ring-gold-400/40 scale-105' : 'border-charcoal-400'
                  }`}>
                    <img src={currentUser.photo} alt="Avatar" className="w-full h-full object-cover" />
                  </div>
                ) : (
                  <Icon className={`w-5 h-5 transition-transform duration-200 ${
                    isActive ? 'stroke-[2.5px] scale-110 text-gold-300' : 'stroke-[1.8px]'
                  }`} />
                )}
              ...
              <span className={`text-[10px] tracking-tight mt-0.5 transition-all ${
                isActive ? 'text-emerald-950 font-extrabold scale-100' : 'text-charcoal-500 font-medium'
              }`}>
                {item.label}
              </span>
            </button>
